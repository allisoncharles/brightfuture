import styles from "../../../styles/FaceAD.module.css";
import AdminNav from "../../../components/AdminNav";
import FaceComponent from "../../../components/FaceComponent";
import { GET_ALL_FACE } from "../../../graphql/queries";
...
const Face = ({ faceRes }) => {
  ...
  return (
    <>
      <NavBar />
      <div className={styles.admin__face}>
        {token && !loading && (
          <>
            <div ref={adminNavRef} className={styles.admin__face__left}>
              <AdminNav />
            </div>
            <div className={styles.admin__face__right}>
...
export async function getServerSideProps({ req, res }) {
  ...
  const faceRes = await client.query({ query: GET_ALL_FACE });
  return {
    props: {
      faceRes: faceRes?.data?.getAllFace ?? [],
    },
  };
}
