...
const Setting = ({ settingRes }) => {
  ...
  return (
    <>
      <NavBar />
      <div className={styles.admin__settings}>
        {token && !loading && (
          <>
            <div ref={adminNavRef} className={styles.admin__settings__left}>
              <AdminNav />
            </div>
            <div className={styles.admin__settings__right}>
              ...
              <SettingComponent settingRes={settingRes} />
            </div>
          </>
        )}
      </div>
...
export async function getServerSideProps({ req, res }) {
  ...
  const settingRes = await client.query({ query: GET_SETTING });
  return {
    props: {
      settingRes: settingRes?.data?.getAllSettings ?? [],
    },
  };
}
