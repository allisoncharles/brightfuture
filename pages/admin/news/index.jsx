...
const News = ({ newsRes }) => {
  ...
  return (
    <>
      <NavBar />
      <div className={styles.admin__news}>
        {token && !loading && (
          <>
            <div ref={adminNavRef} className={styles.admin__news__left}>
              <AdminNav />
            </div>
            <div className={styles.admin__news__right}>
...
export async function getServerSideProps({ req, res }) {
  ...
  const newsRes = await client.query({ query: GET_ALL_NEWS });
  return {
    props: {
      newsRes: newsRes?.data?.getAllNews ?? [],
    },
  };
}
